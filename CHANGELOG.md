...
- **Subclasses of `RegistrationSettings` can once again override the nav label and page title.** v1.4.0's `getNavigationLabel()` / `getTitle()` returned the translated string unconditionally, ignoring `static::$navigationLabel` / `static::$title`. Both getters now check the static property first and fall back to the translation, restoring the documented subclass-and-override pattern (e.g. for `settingsPage()` Shield gating).
